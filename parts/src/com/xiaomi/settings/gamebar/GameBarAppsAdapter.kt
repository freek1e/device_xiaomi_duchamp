...
/**
 * RecyclerView adapter that displays a list of installed applications for selection
 * into the GameBar auto-enable list.
 *
 * Battery / GC optimisation notes:
 * - [ViewHolder] uses direct field references (lateinit) instead of repeated `findViewById`
 *   on each bind call — consistent with the original Java approach.
 * - The [onAppClick] callback is a SAM interface to minimise anonymous-class overhead.
 */
class GameBarAppsAdapter(
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.itemView.setOnClickListener { listener.onAppClick(appInfo) }
    }
